import { useState } from 'react'
import { Modal } from '../Common/Modal'
import { TechDocument } from '../../types/tech_document'
import { deleteTechDocument } from '../../api/tech_documents'

interface DeleteTechDocumentModalProps {
  isOpen: boolean
  onClose: () => void
  document: TechDocument | null
  onSuccess: () => void
}

export function DeleteTechDocumentModal({
  isOpen,
  onClose,
  document,
  onSuccess,
}: DeleteTechDocumentModalProps) {
  const [isDeleting, setIsDeleting] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const handleDelete = async () => {
    if (!document) return

    setIsDeleting(true)
    setError(null)

    try {
      await deleteTechDocument(document.id)
      onSuccess()
    } catch (err: any) {
      const message = err.response?.data?.detail || 'Ошибка при удалении документа'
      setError(message)
    } finally {
      setIsDeleting(false)
    }
  }

  const handleClose = () => {
    setError(null)
    onClose()
  }

  if (!document) return null

  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Удалить документ">
      <div className="space-y-4">
        <p className="text-gray-600">
          Удалить документ "{document.filename}"?
        </p>

        {error && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            <p className="text-sm text-red-600">{error}</p>
          </div>
        )}

        <div className="flex justify-end gap-3">
          <button
            onClick={handleClose}
            className="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
            disabled={isDeleting}
          >
            Отмена
          </button>
          <button
            onClick={handleDelete}
            disabled={isDeleting}
            className="px-4 py-2 text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isDeleting ? 'Удаление...' : 'Удалить'}
          </button>
        </div>
      </div>
    </Modal>
  )
}
